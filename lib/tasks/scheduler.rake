...
task :save_game_result => :environment do

  require 'date'

  # 日付指定(本番では今日)
  theday = Date.new(2021, 10, 15)
  year = theday.year
  month = theday.month
  day = theday.day
  ...
  theday_cards = schedule.grep(/#{month}月#{day}日/).to_s.gsub(/\d+月\d+日|\(日\)|楽天生命パーク/, '').delete('a-z', 'A-C', 'E-Z').gsub(/巨|ヤ|D|中|阪|広|日|楽|西|ロ|オ|ソ/,
                                  '巨' => 'g',
                                  'ヤ' => 's',
                                  'D' => 'yb',
                                  '中' => 'd',
